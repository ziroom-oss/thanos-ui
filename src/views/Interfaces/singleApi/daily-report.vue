    ...
    <el-container>
        <el-header>
            <div style="color: white; font-size:15px" align="left"><i class="el-icon-s-promotion"></i>
                单接口日报 - {{this.$route.params.businessName}}
            </div>
        </el-header>
        <el-main>
            <el-row :gutter="20">
                ...
                <el-col :xs="24" :sm="24" :lg="18">
                    <div class="chart-wrapper">
                        <stack-bar-chart :chartData="chartData['stackBarChart']"></stack-bar-chart>
                    </div>
                </el-col>
                <el-col :xs="24" :sm="24" :lg="24">
                    <div style=" background: #fff;padding: 16px 16px 0;">
                        ...
                        <el-select v-show="postData.recordResult === 'FAILURE'" v-model="postData.exceptionType"
                                   filterable clearable
                                   @change="triggerTableFilter"
                                   placeholder="请选择用例结果" value="">
                            <el-option
                        ...
                        <el-table :data="tableData" v-loading="queryLoading">
                                ...
                                <template slot-scope="scope">
                                    <el-form label-position="left" inline class="table-expand">
                                        ...
                                        <el-form-item label="预期结果:">
                                            <el-table size="small" :data="scope.row.assertRecordList" >
                                                <el-table-column type="index" width="50" label="序号">
                                                </el-table-column>
                                                <el-table-column width="100" prop="assertResult" label="运行状态">
                                                    <template slot-scope="props">
                                                        <label style="color: yellowgreen"  v-if="props.row.assertResult == 'SUCCESS'">SUCCESS</label>
                                                        <label style="color: #ff6670" v-if="props.row.assertResult == 'FAILURE'">FAILURE</label>
                                                    </template>
                                                </el-table-column>
                                                <el-table-column width="800" prop="assertContent" label="断言内容">
                                                </el-table-column>
                                            </el-table>
                                        </el-form-item>
                            ...
                            <el-table-column label="运行结果" align="center">
                                <template slot-scope="scope">
                                    <el-tag :type="scope.row.recordResult === 'SUCCESS'?'success':'danger'"
                                            size="medium">{{scope.row.recordResult}}
                                    </el-tag>
                                </template>
                            </el-table-column>
...
export default {
  ...
  methods: {
    getRouterParam () {
      if (this.$route.params.exceptionType !== null) {
        this.postData.recordResult = 'FAILURE'
        this.postData.exceptionType = this.$route.params.exceptionType
      }
    },
    getChartData () {
      fetchDailyReportSummary({
    ...
    triggerTableFilter () {
      if (this.postData.recordResult !== 'FAILURE') {
        this.postData.exceptionType = ''
      } else {
        if (this.postData.exceptionType === '') {
          this.postData.exceptionType = '业务代码异常'
        }
      }
      ...
      fetchDailyReportTableData(this.queryParam, this.postData).then(response => {
        ...
        if (response.success) {
          ...
          this.total = data.total
          this.creatorOptions = [].concat(response.data.creatorOptions)
          debugger
        }
      })
    }
